...
| `scoped` | Included only when the consumer’s change set matches `applies_when`. Plan-validation matching is defined in `canon/rubrics/plan/plan-rubric.v1.md` (AST-928); code-review matching may be defined by the code rubric (AST-929). This schema only labels the fields. |
