...
For each pattern id cited in the plan / parent Architectural definition ("Patterns to reuse"):

1. Resolve the id under `canon/patterns/**`. Not found, or found with `status` other than `approved` → **fix-now** (invalid citation — unapproved or nonexistent pattern id).
2. Score the diff against the pattern's `# Solution shape` and `## When not to use`: `conforms` | `violates` | `needs-discussion`.
3. If the Architectural definition states "no established pattern applies" but the diff's shape matches an approved catalog entry's `# Problem` (same recurring situation) → **discuss**: uncited pattern match.

List every scored id under `## Pattern conformance`: `id | verdict | one-line`. Statute-namespaced items (e.g. `astral.idioms.*`) are **statutes**, not catalog patterns — already scored via C1–C4, and do not appear in this table.
...
Review Posted forbidden unless Linear comment starts with `[code-rubric] revision=2` and includes complete `## Statutes checked`.
...
1. First line: `[code-rubric] revision=2`
2. `**Rubric:** code-rubric.v2`
3. `**Ticket:** AST-NNN`
...
7. `## Pattern conformance` — table `id | verdict | one-line` per cited pattern id, or `none cited`
