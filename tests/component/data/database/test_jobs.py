...
# Branches: source default/R/W/backfill; candidate-scoped land dedupe helpers (AST-1469).
class TestAst1469JobSourceAndMeteoriteDedupe:
    """AST-1469: job.source column + save_job R/W + find_* meteorite dedupe helpers."""

    def test_insert_defaults_gazed_and_accepts_meteorite(self, seeded_db) -> None:
        from src.utils.config import JOB_SOURCE_DEFAULT, JOB_SOURCE_METEORITE

        db = seeded_db
        db.save_company("acme", state="IMPORTED")
        assert db.save_job("job-default", company="acme", state="NEW") is True
        assert db.get_job("job-default")["source"] == JOB_SOURCE_DEFAULT
        assert db.save_job(
            "job-met", company="acme", state="METEORITE_NEW", source=JOB_SOURCE_METEORITE
        ) is True
        assert db.get_job("job-met")["source"] == JOB_SOURCE_METEORITE
        with pytest.raises(ValueError):
            db.save_job("job-bad", company="acme", state="NEW", source="inbox")

    def test_update_source_and_reject_unknown(self, seeded_db) -> None:
        from src.utils.config import JOB_SOURCE_METEORITE

        db = seeded_db
        db.save_company("acme", state="IMPORTED")
        db.save_job("job-u", company="acme", state="NEW")
        db.save_job("job-u", source=JOB_SOURCE_METEORITE)
        assert db.get_job("job-u")["source"] == JOB_SOURCE_METEORITE
        with pytest.raises(ValueError):
            db.save_job("job-u", source="gazed-ish")

    def test_backfill_null_source_to_gazed(self, seeded_db) -> None:
        from src.utils.config import JOB_SOURCE_DEFAULT

        db = seeded_db
        db.save_company("acme", state="IMPORTED")
        db.save_job("job-bf", company="acme", state="NEW")
        conn = db._get_connection()
        try:
            conn.execute("UPDATE job SET source = NULL WHERE astral_job_id = ?", ("job-bf",))
            conn.commit()
        finally:
            conn.close()
        db._job_source_backfill_applied = False
        db._job_schema_ensured = False
        row = db.get_job("job-bf")
        assert row is not None
        assert row["source"] == JOB_SOURCE_DEFAULT

    def test_find_by_company_job_id_scoped_and_min_length(self, sqlite_in_memory) -> None:
        from src.utils.config import METEORITE_CONFIG

        db = sqlite_in_memory
        min_chars = int(METEORITE_CONFIG["min_company_job_id_match_chars"])
        long_id = "X" * min_chars
        short_id = "Y" * (min_chars - 1)
        db.save_company("co-a", state="IMPORTED", candidate_id="cand-a")
        db.save_company("co-b", state="IMPORTED", candidate_id="cand-b")
        db.save_job("ja", company="co-a", state="NEW", company_job_id=long_id)
        db.save_job("jb", company="co-b", state="NEW", company_job_id=long_id)
        db.save_job("js", company="co-a", state="NEW", company_job_id=short_id)
        hit = db.find_candidate_job_by_company_job_id("cand-a", long_id)
        assert hit is not None and hit["astral_job_id"] == "ja"
        assert db.find_candidate_job_by_company_job_id("cand-a", short_id) is None
        assert db.find_candidate_job_by_company_job_id("cand-b", long_id)["astral_job_id"] == "jb"
        assert db.find_candidate_job_by_company_job_id("", long_id) is None
        assert db.find_candidate_job_by_company_job_id("cand-a", "") is None

    def test_find_by_job_link_and_dedupe_match_order(self, sqlite_in_memory) -> None:
        db = sqlite_in_memory
        link = "https://jobs.example.com/role/1"
        db.save_company("co-a", state="IMPORTED", candidate_id="cand-a")
        db.save_job(
            "by-link",
            company="co-a",
            state="NEW",
            job_link=link,
            company_job_id="LINKONLY1",
        )
        assert db.find_candidate_job_by_job_link("cand-a", link)["astral_job_id"] == "by-link"
        assert db.find_candidate_job_by_job_link("cand-a", link + "/") is None
        assert db.find_candidate_job_by_job_link("", link) is None
        # company_job_id strategy wins before job_link when both present.
        db.save_job(
            "by-id",
            company="co-a",
            state="NEW",
            company_job_id="DEDUPID01",
            job_link="https://other.example/x",
        )
        match = db.find_meteorite_dedupe_match(
            "cand-a", company_job_id="DEDUPID01", job_link=link
        )
        assert match["astral_job_id"] == "by-id"
        link_only = db.find_meteorite_dedupe_match(
            "cand-a", company_job_id=None, job_link=link
        )
        assert link_only["astral_job_id"] == "by-link"
        assert db.find_meteorite_dedupe_match("cand-a") is None
        with pytest.raises(ValueError, match="candidate_id"):
            db.find_meteorite_dedupe_match("")
