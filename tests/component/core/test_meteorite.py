...
@pytest.mark.skipif(
    not hasattr(meteorite_mod, "run_land_meteorite"),
    reason="AST-1560 land runner not on this publish tip",
)
class TestAst1693RunLandBotBlocked:
    """AST-1693: claim BOT_BLOCKED; contentful land with http job_link; empty stays."""

    @pytest.mark.asyncio
    async def test_contentful_bot_blocked_lands_with_http_link(
        self, sqlite_in_memory, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        db = sqlite_in_memory
        cid = "cand-1693-land"
        ...
        def _save(_cid, **kwargs):
            ...
            return {
                "outcome": METEORITE_CONFIG["land_outcome_created"],
                "astral_job_id": "job-1693-land",
            }

        monkeypatch.setattr(meteorite_mod.tracker, "save_meteorite_job", _save)
        out = await meteorite_mod.run_land_meteorite(
            _ingress_task(
                batch_id="land-1693-bb",
                task_key=METEORITE_INGRESS_DISPATCH_CONFIG["land_task_key"],
            )
        )
        assert out["total_passed"] == 1
    ...
    async def test_empty_bot_blocked_left_for_estelle(self, sqlite_in_memory) -> None:
        ...
        row_id = _insert_meteorite_row(
            ...
            state="BOT_BLOCKED",
            content="",
            link="https://example.test/job/empty",
        )
        out = await meteorite_mod.run_land_meteorite(
            _ingress_task(
                batch_id="land-1693-empty",
                task_key=METEORITE_INGRESS_DISPATCH_CONFIG["land_task_key"],
            )
        )
        ...
        assert not row.get("astral_job_id")
...
class TestAst1693NotifySkipsContentful:
    ...
    @pytest.mark.asyncio
    async def test_skips_contentful_bot_blocked(
        self, sqlite_in_memory, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        db = sqlite_in_memory
        ...
        db.save_candidate(
            cid,
            state="NEW_CANDIDATE",
            candidate_data={"name": "N", "contact": {"slack_user_id": "U-1693"}},
        )
        row_id = _insert_meteorite_row(
            db,
            cid,
            state="BOT_BLOCKED",
            content="Paste-ready JD " + ("x" * 40),
            link="https://jobs.example/blocked-contentful",
        )
        post = MagicMock()
        monkeypatch.setattr(
            meteorite_mod,
        ...
        monkeypatch.setattr("src.core.contact.contact_post_message", post)
        out = await meteorite_mod.run_notify_meteorite_bot_blocked(
            _notify_task(batch_id="notify-1693-skip")
        )
        assert out["total_passed"] == 0
        row = db.get_meteorite(row_id)
        assert row["state"] == "BOT_BLOCKED"
        assert not row.get("estelle_notified_at")
        post.assert_not_called()
