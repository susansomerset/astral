...
class TestAst1693PersistHttpJobLink:
    """AST-1693: link-only http(s) job_link write; non-http breadcrumbs no-op."""

    def test_writes_https_job_link(self, sqlite_in_memory) -> None:
        db = sqlite_in_memory
        cid = "cand-1693-http"
        db.save_candidate(cid, state="NEW_CANDIDATE", candidate_data={"name": "H"})
        db.save_company("co-1693-http", state="IGNORE", candidate_id=cid)
        db.save_job("job-1693-http", company="co-1693-http", state="METEORITE_NEW", job_link="", candidate_id=cid)
        tracker_mod.persist_http_job_link("job-1693-http", " https://jobs.example.test/1 ")
        assert db.get_job("job-1693-http")["job_link"] == "https://jobs.example.test/1"

    def test_non_http_breadcrumb_noop(self, sqlite_in_memory) -> None:
        db = sqlite_in_memory
        cid = "cand-1693-crumb"
        db.save_candidate(cid, state="NEW_CANDIDATE", candidate_data={"name": "C"})
        db.save_company("co-1693-crumb", state="IGNORE", candidate_id=cid)
        db.save_job("job-1693-crumb", company="co-1693-crumb", state="METEORITE_NEW", job_link="", candidate_id=cid)
        tracker_mod.persist_http_job_link("job-1693-crumb", "email:recruiter@example.com")
        assert not (db.get_job("job-1693-crumb").get("job_link") or "").strip()


class TestAst1693SaveMeteoriteDuplicateLinkBackfill:
    """AST-1693: meteorite duplicate-skip backfills empty job_link; never clobbers."""

    def test_backfills_empty_job_link_on_duplicate_skip(self, sqlite_in_memory) -> None:
        ...
        db.save_job("existing-1693", company=short, state=METEORITE_CONFIG["job_create_state"],
                    source=JOB_SOURCE_METEORITE, company_job_id="SKIPLAND1693A", job_title="Old",
                    job_link="", candidate_id=cid)
        link = "https://jobs.example.test/backfill"
        out = tracker_mod.save_meteorite_job(cid, company=short, company_job_id="SKIPLAND1693A",
                                             job_title="Ignored", job_link=link)
        assert out["outcome"] == METEORITE_CONFIG["land_outcome_duplicate_skip"]
        row = db.get_job("existing-1693")
        assert row["job_link"] == link and row["job_title"] == "Old"

    def test_does_not_clobber_existing_job_link(self, sqlite_in_memory) -> None:
        from src.utils.config import JOB_SOURCE_METEORITE, METEORITE_CONFIG
        db = sqlite_in_memory
        cid = "cand-1693-keep"
        short = METEORITE_CONFIG["short_name_template"].format(candidate_id=cid)
        ...
        db.save_company(short, state="IGNORE", candidate_id=cid)
        kept = "https://jobs.example.test/kept"
        db.save_job("existing-1693-keep", company=short, state=METEORITE_CONFIG["job_create_state"],
                    source=JOB_SOURCE_METEORITE, company_job_id="KEEP1693LINKX", job_title="Old",
                    job_link=kept, candidate_id=cid)
        out = tracker_mod.save_meteorite_job(cid, company=short, company_job_id="KEEP1693LINKX",
