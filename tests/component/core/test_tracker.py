...
# Branches: create / gazed-supersede / meteorite skip; set_job_source one-way; Style D (AST-1469).
class TestAst1469SaveMeteoriteJob:
    """AST-1469: Tracker save_meteorite_job + set_job_source one-way enforcement."""

    def _seed_meteorite_company(self, db, cid: str = "cand-1469") -> str:
    ...
    def test_rejects_empty_candidate_or_company(self, sqlite_in_memory) -> None:
        with pytest.raises(ValueError, match="candidate_id"):
            tracker_mod.save_meteorite_job("", company="meteorite-x")
        with pytest.raises(ValueError, match="company"):
            tracker_mod.save_meteorite_job("cand-x", company="  ")

    def test_create_new_with_employer_name(self, sqlite_in_memory) -> None:
        from src.utils.config import JOB_SOURCE_METEORITE, METEORITE_CONFIG

        db = sqlite_in_memory
        cid = "cand-create"
        short = self._seed_meteorite_company(db, cid)
        out = tracker_mod.save_meteorite_job(
            cid,
            company=short,
            company_job_id="CREATEID1",
            job_title="Role",
            job_link="https://jobs.example.com/create",
            employer_name="Acme Corp",
            debug=False,
        )
        assert out["outcome"] == METEORITE_CONFIG["land_outcome_created"]
        assert out["source"] == JOB_SOURCE_METEORITE
        ...
        assert row["latest_score"] == float(METEORITE_CONFIG["job_create_latest_score"])
        emp_key = METEORITE_CONFIG["employer_name_job_data_key"]
        assert row["job_data"][emp_key] == "Acme Corp"

    def test_duplicate_skip_existing_meteorite(self, sqlite_in_memory) -> None:
        from src.utils.config import JOB_SOURCE_METEORITE, METEORITE_CONFIG

        db = sqlite_in_memory
        cid = "cand-skip"
        short = self._seed_meteorite_company(db, cid)
        first = tracker_mod.save_meteorite_job(
            cid,
            company=short,
            company_job_id="SKIPID001",
            job_title="A",
            job_link="https://jobs.example.com/skip",
        )
        second = tracker_mod.save_meteorite_job(
            cid,
            company=short,
            company_job_id="SKIPID001",
            job_title="B-should-not-write",
            employer_name="Nope",
        )
        assert second["outcome"] == METEORITE_CONFIG["land_outcome_duplicate_skip"]
        assert second["astral_job_id"] == first["astral_job_id"]
        assert second["source"] == JOB_SOURCE_METEORITE
        row = db.get_job(first["astral_job_id"])
        assert row["job_title"] == "A"
        emp_key = METEORITE_CONFIG["employer_name_job_data_key"]
        assert emp_key not in (row.get("job_data") or {})

    def test_supersede_gazed_any_prior_state_keeps_company(self, sqlite_in_memory) -> None:
        from src.utils.config import JOB_SOURCE_DEFAULT, JOB_SOURCE_METEORITE, METEORITE_CONFIG

        db = sqlite_in_memory
        cid = "cand-super"
        short = self._seed_meteorite_company(db, cid)
        # Gazed job on a roster company — supersede must keep that company.
        db.save_company("gazeco", state="IMPORTED", candidate_id=cid)
        db.save_job(
            "gazed-1",
            company="gazeco",
            state="FAILED_JD",
            source=JOB_SOURCE_DEFAULT,
            company_job_id="SUPERID01",
            job_title="Old",
            job_link="https://jobs.example.com/super",
        )
        out = tracker_mod.save_meteorite_job(
            cid,
            company=short,
            ...
            job_title="New Title",
            job_link="https://jobs.example.com/super",
            employer_name="Known Co",
        )
        assert out["outcome"] == METEORITE_CONFIG["land_outcome_superseded"]
        assert out["astral_job_id"] == "gazed-1"
        assert out["source"] == JOB_SOURCE_METEORITE
        row = db.get_job("gazed-1")
        assert row["company"] == "gazeco"
        assert row["source"] == JOB_SOURCE_METEORITE
        assert row["state"] == METEORITE_CONFIG["job_create_state"]
        assert row["job_title"] == "New Title"
        emp_key = METEORITE_CONFIG["employer_name_job_data_key"]
        assert row["job_data"][emp_key] == "Known Co"

    def test_set_job_source_one_way(self, sqlite_in_memory) -> None:
        from src.utils.config import JOB_SOURCE_DEFAULT, JOB_SOURCE_METEORITE

        db = sqlite_in_memory
        db.save_company("acme", state="IMPORTED")
        db.save_job("j1", company="acme", state="NEW", source=JOB_SOURCE_DEFAULT)
        tracker_mod.set_job_source("j1", JOB_SOURCE_METEORITE)
        assert db.get_job("j1")["source"] == JOB_SOURCE_METEORITE
        with pytest.raises(ValueError, match="meteorite → gazed"):
            tracker_mod.set_job_source("j1", JOB_SOURCE_DEFAULT)
        with pytest.raises(ValueError, match="Job not found"):
            tracker_mod.set_job_source("missing", JOB_SOURCE_METEORITE)

    def test_debug_true_emits_style_d_false_silent(
        ...
        tracker_mod.save_meteorite_job(
            cid,
            company=short,
            company_job_id="DEBUGID01",
            job_title="D",
            debug=True,
        )
        log.set_debug_flag.assert_called_with(True)
        assert log.debug_index.call_args.kwargs["func"] == "tracker.save_meteorite_job"
        assert log.debug_index.call_args.kwargs["outcome"] == METEORITE_CONFIG[
            "land_outcome_created"
        ]
    ...
    def test_insert_bounce_returns_duplicate_skip(
        self, sqlite_in_memory, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from src.utils.config import JOB_SOURCE_METEORITE, METEORITE_CONFIG
        ...
        def _save(*args, **kwargs):
            calls["n"] += 1
            # First call is the create INSERT — force identity bounce.
            if calls["n"] == 1 and kwargs.get("merge") is False:
                return False
        ...
        monkeypatch.setattr(
            tracker_mod.database,
            "find_meteorite_dedupe_match",
            MagicMock(side_effect=[None, existing]),
        )
        out = tracker_mod.save_meteorite_job(
            cid, company=short, company_job_id="BOUNCEID1", job_title="B"
        )
        assert out["outcome"] == METEORITE_CONFIG["land_outcome_duplicate_skip"]
        assert out["astral_job_id"] == "existing-bounce"

    def test_debug_on_supersede_emits_matched_and_source(
        self, sqlite_in_memory, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from src.utils.config import JOB_SOURCE_DEFAULT, METEORITE_CONFIG
        ...
        db.save_job(
            ...
            state="NEW",
            source=JOB_SOURCE_DEFAULT,
            company_job_id="DBGSUPER1",
        )
        log = MagicMock()
        monkeypatch.setattr(tracker_mod, "get_logger", lambda _name: log)
        out = tracker_mod.save_meteorite_job(
            cid, company=short, company_job_id="DBGSUPER1", debug=True
        )
        assert out["outcome"] == METEORITE_CONFIG["land_outcome_superseded"]
        detail_joined = " ".join(str(c.args[0]) for c in log.debug_detail.call_args_list)
        assert "matched_id=gazed-dbg" in detail_joined
        assert "source_before=" in detail_joined

    def test_unexpected_match_source_raises(self, sqlite_in_memory, monkeypatch: pytest.MonkeyPatch) -> None:
        db = sqlite_in_memory
        cid = "cand-weird"
        short = self._seed_meteorite_company(db, cid)
        monkeypatch.setattr(
            tracker_mod.database,
            "find_meteorite_dedupe_match",
            MagicMock(return_value={"astral_job_id": "x", "source": "legacy"}),
        )
        with pytest.raises(ValueError, match="Unexpected job source"):
            tracker_mod.save_meteorite_job(cid, company=short, company_job_id="WEIRDID01")

    def test_insert_bounce_via_identity_then_runtime_error(
        self, sqlite_in_memory, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        db = sqlite_in_memory
        cid = "cand-bounce2"
        short = self._seed_meteorite_company(db, cid)
        monkeypatch.setattr(
            tracker_mod.database, "save_job", MagicMock(return_value=False)
        )
        monkeypatch.setattr(
            tracker_mod.database,
            "find_meteorite_dedupe_match",
        ...
        monkeypatch.setattr(
            tracker_mod.database,
            "get_job_id_by_identity",
            MagicMock(return_value="id-bounce"),
        )
        monkeypatch.setattr(
            tracker_mod.database,
            "get_job",
            MagicMock(return_value={"astral_job_id": "id-bounce", "source": "meteorite"}),
        )
        out = tracker_mod.save_meteorite_job(
            cid, company=short, company_job_id="IDBOUNCE1", job_title="T"
        )
        assert out["astral_job_id"] == "id-bounce"

        monkeypatch.setattr(
            tracker_mod.database, "get_job_id_by_identity", MagicMock(return_value=None)
        )
        with pytest.raises(RuntimeError, match="insert failed"):
            tracker_mod.save_meteorite_job(
                cid, company=short, company_job_id="IDBOUNCE2", job_title="T2"
            )

    def test_supersede_by_link_only_omits_optional_column_writes(self, sqlite_in_memory) -> None:
        from src.utils.config import JOB_SOURCE_DEFAULT, JOB_SOURCE_METEORITE, METEORITE_CONFIG
        ...
        short = self._seed_meteorite_company(db, cid)
        db.save_company("gazeco", state="IMPORTED", candidate_id=cid)
        link = "https://jobs.example.com/link-only"
        db.save_job(
            "gazed-link",
            company="gazeco",
            state="NEW",
            source=JOB_SOURCE_DEFAULT,
            job_title="Keep Title",
            job_link=link,
            company_job_id="KEEPID001",
        )
        # Match via job_link only — no company_job_id / title / link kwargs on save.
        out = tracker_mod.save_meteorite_job(cid, company=short, job_link=link)
        assert out["outcome"] == METEORITE_CONFIG["land_outcome_superseded"]
        row = db.get_job("gazed-link")
        assert row["source"] == JOB_SOURCE_METEORITE
        assert row["job_title"] == "Keep Title"
        assert row["company_job_id"] == "KEEPID001"
        assert row["job_link"] == link

    def test_supersede_missing_row_after_save_raises(
        self, sqlite_in_memory, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from src.utils.config import JOB_SOURCE_DEFAULT
        ...
        db.save_job(
            ...
            state="NEW",
            source=JOB_SOURCE_DEFAULT,
            company_job_id="GONEID001",
        )
        match = db.get_job("gazed-gone")
        monkeypatch.setattr(
            tracker_mod.database,
            "find_meteorite_dedupe_match",
            MagicMock(return_value=match),
        )
        monkeypatch.setattr(tracker_mod.database, "save_job", MagicMock(return_value=True))
        monkeypatch.setattr(tracker_mod.database, "get_job", MagicMock(return_value=None))
        with pytest.raises(RuntimeError, match="supersede missing"):
            tracker_mod.save_meteorite_job(cid, company=short, company_job_id="GONEID001")

    def test_create_missing_row_after_save_raises(
        self, sqlite_in_memory, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        db = sqlite_in_memory
        cid = "cand-create-miss"
        short = self._seed_meteorite_company(db, cid)
        monkeypatch.setattr(
            tracker_mod.database, "find_meteorite_dedupe_match", MagicMock(return_value=None)
        )
        monkeypatch.setattr(tracker_mod.database, "save_job", MagicMock(return_value=True))
        monkeypatch.setattr(tracker_mod.database, "get_job", MagicMock(return_value=None))
        with pytest.raises(RuntimeError, match="missing after save"):
            tracker_mod.save_meteorite_job(
                cid, company=short, company_job_id="CREATEMISS", job_title="X"
            )
