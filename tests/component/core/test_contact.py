...
# Branches: contact_land_meteorite wrapper; Estelle land_calls → wrapper (AST-1471).
class TestAst1471ContactLandMeteorite:
    """AST-1471: Contact sync land entry + Estelle land_calls loop."""

    def test_contact_land_meteorite_runs_core(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from src.utils.config import METEORITE_CONFIG
        import src.core.meteorite as meteorite_mod
        ...
        out = contact_mod.contact_land_meteorite(
            "c1", text="listing text", job_link="https://x.example/j", debug=True
        )
        assert out["outcome"] == METEORITE_CONFIG["land_outcome_created"]
        assert seen["cid"] == "c1"
        assert seen["kwargs"]["text"] == "listing text"
        assert seen["kwargs"]["job_link"] == "https://x.example/j"
        assert seen["kwargs"]["debug"] is True

    def test_estelle_land_calls_invokes_wrapper(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from src.utils.config import METEORITE_CONFIG

        helper = TestAst1073ContactEstelleTurnLoop()
        helper.setup_method()
        deps = helper._patch_turn_deps(
            monkeypatch,
            do_task_result={
                "success": True,
                "conversational_outcome": "success",
                "agent_performance": {"status": "success"},
                "parsed_response": {
                    "reply": "Got that listing",
                    "land_calls": [
                        {"text": "Engineer role", "job_link": "https://jobs.example/1"},
                        {"scraps": [{"text": "Second", "employer_name": "Acme"}]},
                    ],
                },
            },
        )
        ...
        out = contact_mod.run_contact_estelle_turn(
            channel="C1", text="here is a job", astral_candidate_id="c1", debug=False
        )
        assert out["ok"] is True
        assert len(out["land_results"]) == 2
        assert all(r.get("ok") for r in out["land_results"])
        assert land.call_count == 2
        assert land.call_args_list[0].kwargs.get("text") == "Engineer role" or (
            land.call_args_list[0].args[0] == "c1"
        )
        # First item: top-level text/link kwargs; second: scraps=
        first_kw = land.call_args_list[0].kwargs
        assert first_kw.get("text") == "Engineer role"
        assert first_kw.get("job_link") == "https://jobs.example/1"
        second_kw = land.call_args_list[1].kwargs
        assert second_kw.get("scraps") == [{"text": "Second", "employer_name": "Acme"}]
        deps["skill"].assert_not_called()

    def test_estelle_land_calls_no_candidate(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        helper = TestAst1073ContactEstelleTurnLoop()
        helper.setup_method()
        helper._patch_turn_deps(
            monkeypatch,
            do_task_result={
                "success": True,
                "conversational_outcome": "success",
        ...
        monkeypatch.setattr(contact_mod, "contact_land_meteorite", land)
        out = contact_mod.run_contact_estelle_turn(channel="C1", text="hi", debug=False)
        assert out["land_results"] == [{"ok": False, "error": "no_candidate"}]
