...
class TestAst1270NestedDraftJobResumeContract:
    """AST-1270: unwrap agent_payload.resume; whitelist base_resume keys; notes metadata."""
    ...
    def test_nested_envelope_validates_and_unwraps_resume(self) -> None:
        # Nested sample shape — resume body + sibling notes metadata (AST-1523).
        ...
        notes = ["Unsupported in materials."]
        parsed: dict[str, Any] = {
...
class TestAst1523EpicCandidateHelpersRemoved:
    """AST-1523: coded-list and adherence validate helpers removed from candidate."""

    def test_hard_contract_helpers_removed(self) -> None:
        assert not hasattr(candidate_mod, "parse_advise_job_resume_coded_advice")
...
class TestAst1633StrengthsOperativeSaveHydrate:
    ...
    def test_dict_path_strips_strengths_keeps_siblings(
        ...
        monkeypatch.setattr(candidate_mod.database, "save_candidate", save)
        spy = _spy_save_artifact(monkeypatch)
        # AST-1652: priorities is catalog-owned too — use deal_breakers as library sibling.
        candidate_mod.save_candidate_data(
            "c1",
            {"context": {"strengths": "drop-me", "deal_breakers": "keep-me"}},
        )
        assert spy == []
        ctx = save.call_args.kwargs["candidate_data"]["context"]
        assert ctx == {"deal_breakers": "keep-me"}
...
class TestAst1652PrioritiesOperativeSaveHydrate:
    """AST-1652: Priorities plain_text operative save + hydrate + library gate."""

    def test_plain_text_rejects_empty_and_non_str(self) -> None:
        with pytest.raises(ValueError, match="plain_text body must be a non-empty string"):
            candidate_mod.save_candidate_data("c1", _PRIORITIES_ARTIFACT_KEY, "")
        with pytest.raises(ValueError, match="plain_text body must be a non-empty string"):
            candidate_mod.save_candidate_data("c1", _PRIORITIES_ARTIFACT_KEY, "   ")
        with pytest.raises(ValueError, match="plain_text body must be a non-empty string"):
            candidate_mod.save_candidate_data("c1", _PRIORITIES_ARTIFACT_KEY, {"x": 1})

    def test_operative_save_writes_current_and_skips_library_blob(self, seeded_db) -> None:
        db = seeded_db
        uid = candidate_mod.save_candidate_data(
            "cand-1", _PRIORITIES_ARTIFACT_KEY, "ship priorities"
        )
        assert uid
        row = db.get_current_artifact("candidate", "cand-1", "priorities")
        assert row is not None
        assert row["artifact_uuid"] == uid
        assert row["artifact_data"] == "ship priorities"
        assert row["current"] == 1
        cd = db.get_candidate("cand-1")["candidate_data"]
        assert "priorities" not in (cd.get("context") or {})

    def test_second_operative_save_retires_prior(self, seeded_db) -> None:
        db = seeded_db
        uid1 = candidate_mod.save_candidate_data(
            "cand-1", _PRIORITIES_ARTIFACT_KEY, "v1 priorities"
        )
        uid2 = candidate_mod.save_candidate_data(
            "cand-1", _PRIORITIES_ARTIFACT_KEY, "v2 priorities"
        )
        assert uid1 != uid2
        current = db.get_current_artifact("candidate", "cand-1", "priorities")
        assert current["artifact_uuid"] == uid2
        assert current["artifact_data"] == "v2 priorities"
        history = db.list_artifacts(
            "candidate", "cand-1", "priorities", current_only=False
        )
        assert len(history) == 2
        assert history[0]["current"] == 0
    ...
    def test_identical_body_keeps_current_uuid(self, seeded_db) -> None:
        ...
        db = seeded_db
        uid1 = candidate_mod.save_candidate_data(
            "cand-1", _PRIORITIES_ARTIFACT_KEY, "same priorities"
        )
        uid2 = candidate_mod.save_candidate_data(
            "cand-1", _PRIORITIES_ARTIFACT_KEY, "same priorities"
        )
    ...
    def test_dict_path_strips_priorities_and_strengths_keeps_siblings(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        save = MagicMock()
        monkeypatch.setattr(
        ...
        ctx = save.call_args.kwargs["candidate_data"]["context"]
        assert ctx == {"deal_breakers": "keep-me"}
        assert "priorities" not in ctx
        assert "strengths" not in ctx

    def test_dict_path_priorities_only_skips_empty_library_write(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        save = MagicMock()
        monkeypatch.setattr(
    ...
    def test_hydrate_overlays_hit_leaves_legacy_on_miss(self, seeded_db) -> None:
        candidate_mod.save_candidate_data(
            "cand-1", _PRIORITIES_ARTIFACT_KEY, "operative priorities"
        )
        cd: dict[str, Any] = {"context": {"priorities": "stale blob"}}
        candidate_mod.hydrate_operative_priorities_for_response("cand-1", cd)
        assert cd["context"]["priorities"] == "operative priorities"

        legacy: dict[str, Any] = {"context": {"priorities": "legacy until re-save"}}
        candidate_mod.hydrate_operative_priorities_for_response("missing-id", legacy)
        assert legacy["context"]["priorities"] == "legacy until re-save"

    def test_hydrate_ignores_non_dict_cd_and_non_str_body(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        candidate_mod.hydrate_operative_priorities_for_response(
            "c1", "not-a-dict"
        )  # type: ignore[arg-type]
        monkeypatch.setattr(
            candidate_mod, "get_candidate_current", lambda cid, key: {"not": "str"}
        )
        cd: dict[str, Any] = {"context": {"priorities": "keep"}}
        candidate_mod.hydrate_operative_priorities_for_response("c1", cd)
        assert cd["context"]["priorities"] == "keep"

    def test_get_candidate_hydrates_priorities(self, seeded_db) -> None:
        candidate_mod.save_candidate_data(
            "cand-1", _PRIORITIES_ARTIFACT_KEY, "from get_candidate"
        )
        row = candidate_mod.get_candidate("cand-1")
        assert row["candidate_data"]["context"]["priorities"] == "from get_candidate"
