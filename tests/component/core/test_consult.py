...
# Branches: validation; do_task fail/success; employer/link fallback; batch_id; Style D (AST-1470).
class TestAst1470EnrichMeteoriteLandPacket:
    """AST-1470: consult.enrich_meteorite_land_packet via qualify_meteorite do_task."""

    @pytest.mark.asyncio
    async def test_rejects_empty_candidate_or_scraps(self) -> None:
        out = await consult_mod.enrich_meteorite_land_packet("", [{"text": "x"}])
        assert out["success"] is False
        assert "candidate_id" in (out.get("error") or "")
        out2 = await consult_mod.enrich_meteorite_land_packet("c1", [])
        assert out2["success"] is False
        assert "scraps" in (out2.get("error") or "")
        out3 = await consult_mod.enrich_meteorite_land_packet(
            "c1", [{"job_link": "", "text": "  "}]
        )
        assert out3["success"] is False
        assert "no usable scraps" in (out3.get("error") or "")

    @pytest.mark.asyncio
    async def test_do_task_failure(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            consult_mod,
            "do_task",
        ...
        assert out["jobs"] == []
        assert out["batch_id"].startswith("qualify_meteorite-land-")

    @pytest.mark.asyncio
    async def test_success_maps_ruth_with_scrap_fallbacks(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        do_task = AsyncMock(
            return_value={
                ...
                "parsed_response": {
                    "jobs": [{
                        "company_job_id": "RUTHID01",
                        "job_title": "Ruth Title",
                        "job_link": "",
                        "jd_text": "Ruth JD " + ("j" * 40),
                        "employer_name": "",
                    }]
                },
            }
        ...
        out = await consult_mod.enrich_meteorite_land_packet(
            "c1",
            [{
                "job_link": "https://scrap.example/j",
                "text": "scrap body",
                "employer_name": "Scrap Co",
            }],
            ctx={"astral_candidate_id": "c1"},
            debug=False,
        )
        ...
        job = out["jobs"][0]
        assert job["job_title"] == "Ruth Title"
        assert job["job_link"] == "https://scrap.example/j"  # Ruth empty → scrap
        assert job["employer_name"] == "Scrap Co"
        assert job["company_job_id"] == "RUTHID01"
        assert out["batch_id"].startswith("qualify_meteorite-land-")
        assert do_task.await_count == 1
        kwargs = do_task.await_args.kwargs
        assert kwargs["task_key"] == "qualify_meteorite"
        assert kwargs["index"] == f"qualify_meteorite_batch_{out['batch_id']}"
        assert "METEORITE JOBS:" in kwargs["live_content"]

    @pytest.mark.asyncio
    async def test_debug_emits_enriched_style_d(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            consult_mod,
        ...
        log = MagicMock()
        monkeypatch.setattr(consult_mod, "logger", log)
        out = await consult_mod.enrich_meteorite_land_packet(
            "c1", [{"text": "z" * 50}], debug=True
        )
        assert out["success"] is True
        assert any(
            c.kwargs.get("func") == "consult.enrich_meteorite_land_packet"
            and c.kwargs.get("outcome") == "enriched"
