...
describe("CandidateDealBreakers", () => {
  beforeEach(() => {
    localStorage.clear()
    mockedApi.mockReset()
    mockedApi.mockImplementation(async (url: string, init?: RequestInit) => {
      ...
      if (url === "/api/candidates/c1" && !init) {
        return { json: async () => ({ candidate_data: { context: { deal_breakers: "no travel" } } }) } as Response
      }
      throw new Error(`unexpected api call: ${url}`)
    })
  })

  it("renders deal breakers context editor", async () => {
    renderWithProviders(<CandidateDealBreakers />)
    await waitFor(() => expect(screen.getByRole("heading", { name: "Deal Breakers" })).toBeInTheDocument())
