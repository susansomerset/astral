...
describe("JobsRecommended", () => {
  ...
  describe("AST-1477 mark applied from Recommended", () => {
    it("shows Applied on Recommended / In Progress / Ready rows", async () => {
      installBaseApiMocks(mockedApi, jobsViewHandler("recommended", sectionedJobs))
      renderWithProviders(<JobsRecommended />)
      await waitFor(() => expect(screen.getByText("Ready Role")).toBeInTheDocument())
      // One Applied per review-like prior row (not PASSED_LIKE — none in fixture).
      expect(screen.getAllByRole("button", { name: "Applied" })).toHaveLength(4)
    })

    it("Applied → notes modal → candidate_action applied → row leaves list", async () => {
      const recommendedJob = {
        ...sectionedJobs[0],
        astral_job_id: "j-rec-apply",
        job_title: "Apply Me",
      }
      let listRows = [recommendedJob]
      installBaseApiMocks(mockedApi, (url, init) => {
        if (typeof url === "string" && url.includes("view=recommended") && !init?.method) {
        ...
        if (
          typeof url === "string" &&
          url === "/api/jobs/j-rec-apply/candidate_action" &&
          init?.method === "POST"
        ) {
          listRows = []
          return jsonResponse({ ok: true })
        }
        return jobsViewHandler("recommended", listRows)(url, init)
      })
      renderWithProviders(<JobsRecommended />)
      await waitFor(() => expect(screen.getByText("Apply Me")).toBeInTheDocument())
      ...
      await waitFor(() =>
        expect(mockedApi).toHaveBeenCalledWith(
          "/api/jobs/j-rec-apply/candidate_action",
    ...
    it("failed Applied transition toasts the API error", async () => {
      const recommendedJob = {
        ...sectionedJobs[0],
        astral_job_id: "j-rec-fail",
        job_title: "Fail Apply",
      }
      installBaseApiMocks(mockedApi, (url, init) => {
        if (
          typeof url === "string" &&
          url === "/api/jobs/j-rec-fail/candidate_action" &&
          init?.method === "POST"
        ) {
          return jsonResponse({ error: "Illegal state transition" }, { ok: false, status: 409 })
        }
        return jobsViewHandler("recommended", [recommendedJob])(url, init)
      })
      renderWithProviders(<JobsRecommended />)
      await waitFor(() => expect(screen.getByText("Fail Apply")).toBeInTheDocument())
      await userEvent.click(screen.getByRole("button", { name: "Applied" }))
      await waitFor(() => expect(screen.getByRole("heading", { name: "Applied" })).toBeInTheDocument())
      await userEvent.click(screen.getByRole("button", { name: "Save" }))
