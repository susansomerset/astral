...
describe("AST-1454 onRefresh after skipped-field Save", () => {
  it("opens Job Detail with editors and Save refreshes the skipped list", async () => {
    const row = {
      ...skippedJob,
      job_link: "https://ex.test/job",
    ...
    let listHits = 0
    installBaseApiMocks(mockedApi, (url, init) => {
      if (url.startsWith("/api/jobs?view=skipped") && !init) {
        listHits += 1
      ...
      if (url === `/api/jobs/${row.astral_job_id}` && init?.method === "PUT") {
        return jsonResponse({
          ...row,
          job_title: "Saved Skipped Title",
          fields_editable: true,
        })
      }
      return jobsViewHandler("skipped", [row])(url, init)
    })
    renderWithProviders(<JobsSkipped />)
    await waitFor(() => expect(screen.getByRole("button", { name: /CANDIDATE_SKIPPED/ })).toBeInTheDocument())
    await userEvent.click(screen.getByRole("button", { name: /CANDIDATE_SKIPPED/ }))
    await userEvent.click(screen.getByText("Skipped Role"))
    await waitFor(() => expect(screen.getByDisplayValue("Skipped Role")).toBeInTheDocument())
    const title = screen.getByDisplayValue("Skipped Role")
    await userEvent.clear(title)
    await userEvent.type(title, "Saved Skipped Title")
    const beforeSaveLists = listHits
    await userEvent.click(screen.getByRole("button", { name: "Save" }))
    await waitFor(() =>
      expect(mockedApi).toHaveBeenCalledWith(
    ...
    await waitFor(() => expect(listHits).toBeGreaterThan(beforeSaveLists))
  })
})
