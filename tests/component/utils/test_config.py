...
# Branches: qualify_meteorite company_job_id required False; omit/null/empty schema (AST-1127).
class TestAst1127QualifyMeteoriteCompanyJobIdOptional:
    def test_schema_required_false(self) -> None:
        from src.utils import config as cfg
    ...
    def test_validate_allows_omit_null_empty(self) -> None:
        ...
        job = {
            ...
            "job_title": "Engineer",
            "job_link": (
                "https://www.dice.com/company-profile/"
                "9f704ad3-7a18-506a-bd5e-6a84e73b7c00"
            ),
            "jd_text": "x" * 50,
        }
        ...
        base["agent_payload"]["jobs"][0] = {**job, "company_job_id": None}
        assert agent_mod._validate_response_schema(base, schema, "qualify_meteorite") is None
...
class TestAst1590JobArtifactCatalogKeys:
    ...
    def test_artifact_config_has_pilot_and_job_keys(self) -> None:
        # ftr-refresh union: Ideal Day (origin/dev) + Backstory (ftr AST-1661).
...
# DEAL_BREAKERS + BIO_SUMMARY + BACKSTORY + IDEAL_DAY artifact keys; COMPANY_SEARCH_TERMS data_field;
...
def _assert_token_sources_typing(
    ...
    """Mirror of src.utils.config AST-1596 / AST-1632 / AST-1648 / AST-1658 / AST-1661 TOKEN_SOURCES asserts."""
    ...
    # AST-1661: BACKSTORY joins the closed artifact-token set.
    ...
    # AST-1658: IDEAL_DAY joins the closed artifact-token set.
...
class TestAst1596TokenCatalogSourceTypeTyping:
    ...
    def test_live_catalog_passes_typing_asserts(self) -> None:
        ...
        # ftr-refresh union: BACKSTORY + IDEAL_DAY artifact (17 / 7 / 27).
        by_type = {
...
class TestAst1632CatalogPlainTextStrengthsToken:
    ...
    _META = {"entity_type", "candidate_scoped", "body_shape", "ingestion_owner"}
    # ftr-refresh union: backstory + ideal_day registered; freeze writing_preferences.
    _CTX_SIBLINGS = (
    ...
    def test_strengths_token_artifact_linkage(self) -> None:
        ...
        # Union tip: BACKSTORY + IDEAL_DAY artifact; writing_preferences data_field.
        ...
        assert "artifact_key" not in cfg.TOKEN_SOURCES["WRITING_PREFERENCES"]
...
class TestAst1648CatalogBioSummaryTokenProfileNav:
    ...
    _META = {"entity_type", "candidate_scoped", "body_shape", "ingestion_owner"}
    # ftr-refresh union: freeze writing_preferences only.
    _CTX_SIBLINGS = (
        "candidate.context.writing_preferences",
    )
    ...
    def test_bio_summary_token_artifact_linkage(self) -> None:
        ...
        # Union tip: BACKSTORY + IDEAL_DAY artifact.
        assert cfg.TOKEN_SOURCES["BACKSTORY"]["source_type"] == "artifact"
        assert (
            cfg.TOKEN_SOURCES["BACKSTORY"]["artifact_key"]
            == "candidate.context.backstory"
...
class TestAst1651CatalogPlainTextPrioritiesToken:
    ...
    def test_priorities_token_artifact_linkage(self) -> None:
        ...
        # Sibling context tokens on AST-1661 tip: BIO_SUMMARY + BACKSTORY artifact.
...
class TestAst1654CatalogPlainTextDealBreakersToken:
    ...
    def test_deal_breakers_token_artifact_linkage(self) -> None:
        ...
        # Sibling context tokens on AST-1661 tip: PRIORITIES + BACKSTORY artifact.
...
class TestAst1658CatalogPlainTextIdealDayToken:
    ...
    _META = {"entity_type", "candidate_scoped", "body_shape", "ingestion_owner"}
    # ftr-refresh union: backstory registered; freeze writing_preferences.
    _CTX_SIBLINGS = (
        "candidate.context.writing_preferences",
    )
    ...
    def test_ideal_day_token_artifact_linkage(self) -> None:
        ...
        # Union tip: sibling context tokens already migrated stay artifact.
        assert cfg.TOKEN_SOURCES["BACKSTORY"]["source_type"] == "artifact"
        assert cfg.TOKEN_SOURCES["WRITING_PREFERENCES"]["source_type"] == "data_field"
...
class TestAst1661CatalogPlainTextBackstoryToken:
    ...
    _META = {"entity_type", "candidate_scoped", "body_shape", "ingestion_owner"}
    # ftr-refresh union: ideal_day registered; freeze writing_preferences.
    _CTX_SIBLINGS = (
        "candidate.context.writing_preferences",
    )
    ...
    def test_backstory_token_artifact_linkage(self) -> None:
        ...
        # Union tip: Ideal Day also artifact; writing_preferences stays data_field.
...
@pytest.mark.skipif(
    "candidate.context.writing_preferences" not in cfg.ARTIFACT_CONFIG,
    reason="AST-1664 product not on this tip (parallel Writing Preferences epic)",
)
