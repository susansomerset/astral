...
| File | Change | Layer |
|------|--------|-------|
| `src/ui/api/api_jobs.py` | Guard `_flatten_grades` when `job_data` is not a dict; catch story/hydrate failures in `detail` so a listed job still returns 200 with payload (empty `agent_story` / unhydrated artifacts as fallback); log via utils logger | ui |
...
**Primary hypothesis (implement against this first)**
...
**Done when:** A concrete traceback for `GET /api/jobs/4a7dbb0c-a1cb-4c1d-ab9d-0c098c8313fc` (or an equivalent RECOMMENDED job that 500s on detail while listing) is captured, and it matches the primary hypothesis (agent_story / agent_data resolve) **or** a Linear parent comment is posted with the actual throw site if different — and work stops until the plan is amended for that site.
...
2. Look up job `4a7dbb0c-a1cb-4c1d-ab9d-0c098c8313fc` via `get_job`. If missing from local DB: upsert that row from the parent AST-1273 Original brief JSON array (parse string `job_data` / `state_history` before save) using existing tracker/database save helpers already used elsewhere — **do not** invent a new migration. Prefer restoring from Susan's live DB if the row reappears on the shared `astral.db` symlink.
...
4. If the traceback is inside `get_entity_agent_story` / agent_data ref resolve / hydrate: proceed to Stage 2 with no plan change.
5. If the traceback is elsewhere: **stop**. Comment on **parent** AST-1273 with the `🛑 Stage 1 blocked` format (throw site, proposed plan amendment). Do not invent a different fix.

⚠️ **Decision:** Diagnosis-first stage is mandatory because the reported job is not currently in this worktree DB; fixing the wrong layer would miss AC1.

### Stage 2: Backend — detail succeeds when the job row exists

**Done when:** For the reported job (or the Stage 1 equivalent that previously 500'd), `GET /api/jobs/<id>` returns **200** with `astral_job_id`, `job_title` / `company`, and `job_data` including Summary fields (`analysis_upshot` when present). Missing job still returns **404** `{"error": "Not found"}`. Server log for the open is not HTTP 500.
...
2. In `src/ui/api/api_jobs.py`:
   ...
   * In `detail`, after `_flatten_grades`:
     ...
     * Wrap `hydrate_job_artifacts_for_display(...)` in `try/except Exception`: on failure log via `get_logger(__name__)` and fall back to `get_job_artifacts(job)` or `{}` so detail still returns the job row.
...
3. Manually verify: reported/equivalent job → 200; unknown id → 404; Flask log shows no 500 for the open.

⚠️ **Decision:** Soft-fail story/hydrate at **core + UI callers**, leave data-layer raises intact (`astral.standards.data-raises-caller-logs`). Empty `agent_story` is acceptable for AC1 — JAR Summary reads `job_data.analysis_upshot`, not story blocks.
...
2. Do **not** edit `JobDetailModal.tsx` in this ticket (In Review shell; out of Recommended report modal scope). If a shared helper is tempting, skip it — one-call-site change only.
...
**Done when:** AC1–AC4 confirmed by hand (or scripted API + UI smoke) on epic worktree; AC5 marked N/A in the Linear stage comment if no `debug=` surfaces were edited.

1. AC1: From Recommended, open the reported/equivalent job → modal shows title/company and Summary content (not not-found empty state).
2. AC2: Server log for that GET is success (not 500).
3. AC3: Missing id → not-found; forced non-404 failure → not the not-found copy.
4. AC4: At least one other Recommended row that already worked still opens.
...
* Ambiguity or Stage 1 mismatch → comment on **parent** AST-1273 with `🛑 Stage N blocked` and wait.
...
**Conf:** `high` — list vs detail code split isolates the failure to story/hydrate; UI not-found mapping is an explicit one-liner; secondary `_flatten_grades` guard matches existing isinstance checks.

**Risk:** `Medium` — soft-failing agent_story could hide broken refs (mitigation: warning/exception logs); wrong catch breadth could mask unrelated detail bugs (mitigation: Stage 1 traceback gate; only wrap hydrate + story, not `get_job`).
