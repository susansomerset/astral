...
**Linear archived:** AST-345 2026-06-03

## Ledger

| when (PT) | ticket | phase | sha | subject |
|---|---|---|---|---|
| 2026-04-23 16:54 | AST-345 | chore | `1a47c88ab` | append review stub |
| 2026-04-23 16:54 | AST-345 | feat | `0e1d790dc` | replace hydrate_response with _decode_payload; add grades_encode |
| 2026-04-23 16:58 | AST-345 | chore | `3694942ad` | add code review findings |
| 2026-04-23 17:02 | AST-345 | fix | `9a3f53d92` | review feedback — stale comment, dict validator branch, grades_j |

## Epic — AST-345

_Archived: 2026-06-03 · Linear URL: https://linear.app/astralcareermatch/issue/AST-345/missing-response-schema-for-qualify-job-listings-and-evaluate-jd · Status at archive: Done · Project: Astral Agent · Assignee: susan · Priority / estimate: None / —_
...
| | file | planned | actual |
|---|---|---|---|
| ✓ | `src/utils/config.py` | Add `grades_json`, `grades_encoded`, `grades_encoded_meta` t | `9a3f53d92` `0e1d790dc` |
| ✓ | `src/core/agent.py` | Add `_GRADE_SEG` (built from config) + `_decode_payload`; wi | `9a3f53d92` `0e1d790dc` |
| ✓ | `src/core/consult.py` | Add `batch_entities` to ctx; remove decode block + `hydrate_ | `9a3f53d92` `0e1d790dc` |
| ✓ | `src/core/tracker.py` | `initialize_job`: flatten nested `job_data` dict into metada | `0e1d790dc` |
| ✓ | `src/core/dispatcher.py` | Move `monitor.auto_run_error()` inside `finally` before `log | `0e1d790dc` |
