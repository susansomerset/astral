<!-- linear-archive: AST-1146 archived 2026-08-07 -->

## Linear archive (AST-1146)

**Archived:** 2026-08-07  
**Linear URL:** https://linear.app/astralcareermatch/issue/AST-1146/uat-create-skips-legitimate-job-via-known-company-job-id-matched29  
...
Manage Email Create on a Dice Saved-jobs email discovered 3 real job-detail links but only created 2. The third (`Lead Systems Analyst` / `https://www.dice.com/job-detail/add50803-2af1-4f26-aba5-3997c9db8905`) logged:

`gazer.meteorite_email_ingest … -> skipped-duplicate`
`reason=known_company_job_id matched=29`

Result: `created=2 skipped=1`. Susan: do not dedupe null job ids; `matched=29` suggests a null/empty or otherwise non-real external id is participating in the match.

## In scope

- [X] `pattern.config.config-block` — `min_company_job_id_match_chars` on `METEORITE_EMAIL_INGEST_CONFIG`
- [X] `astral.config.config-source-of-truth` / `astral.standards.no-hardcoded-sets` — match floor in config, not inline SQL magic
- [X] `astral.standards.in-scope-only` — candidate-scoped id helper + config only; Create path benefits via existing gazer call
- [X] Parent AC: real job-detail links create; false `known_company_job_id` skips from null/short junk ids removed; true same-id / exact `job_link` dedupe preserved (candidate companies only)

## Considered but excluded

* Disable all dedupe / swallow skip without match-rule fix
* Treat short numeric substrings as valid external ids
* Re-introduce global (cross-candidate) id matching
* UUID-only match (Dice-exclusive risk) — use length floor instead
* Change global `text_matches_known_company_job_id` (unused by Create)
* HTML normalize (AST-1131), qualify_meteorite (AST-1133), gaze_email redesign

## Expected

All three legitimate job-detail links create meteorite jobs (or skip only when the same non-empty match-eligible `company_job_id` or exact `job_link` already exists for this candidate’s companies). Null/empty/`LENGTH < min` `company_job_id` values never cause a skip.
...
> Re-running Create on the same message skips already-known links/ids without duplicating rows (existing dedupe behavior preserved).
>
...
> Create on a Dice “Saved jobs” multi-card HTML email (same shape as the Original brief) creates **only** jobs for real job-detail postings present in that list — **zero** jobs whose `job_link` is an SVG/namespace/spec URL

(All real postings in the list must be created; false duplicate skips violate that.)

## Diagnosis

* **Hypothesis:** Candidate-scoped `known_company_job_id` substring match treats null/empty and/or tiny non-identifier values (logged as `matched=29`) as known ids, so a new Dice posting’s visible text false-matches and Create skips a legitimate third job.
* **Correct outcome:** Create records all three Dice job-detail links as new meteorite jobs unless the same non-empty external id or exact job link already exists under this candidate’s companies; null/empty `company_job_id` rows never participate in id dedupe.
* **Wrong fix to avoid:** Disable all dedupe; swallow skip without fixing match rules; treat short numeric substrings as valid external ids; re-introduce global (cross-candidate) id matching.
* **Related siblings / contracts:** AST-1132 (job-link hygiene + create skip / candidate-scoped dedupe); AST-1131 (HTML normalize); AST-1061 (original Playwright + dedupe). Exact `job_link` dedupe for true re-ingest must still hold.

## Boundaries

* This bug does **not** change: HTML normalize (AST-1131), qualify_meteorite apply (AST-1133), gaze_email redesign.
* "No more skipped-duplicate log" alone is **not** done — Parent AC + Correct outcome must hold (three legitimate links create unless true same-job dedupe).

## Notes for planning

Plan: `docs/features/meteorite/ast-1146-uat-create-skips-null-company-job-id-dedupe.md` on publish ref below.

## Git branch (authoritative)

Per orientation § Branch law: parent `ftr/AST-1130-manage-email-create-button-for-job-lists-isnt-working`, child `sub/AST-1130/AST-1146-uat-create-skips-null-company-job-id-dedupe`. Seeded at fix-uat from origin/ftr parent.

### Comments

#### radia — 2026-08-03T00:15:58.084Z
[code-rubric] revision=1
**Rubric:** code-rubric.v1
**Ticket:** AST-1146
**Publish ref:** origin/sub/AST-1130/AST-1146-uat-create-skips-null-company-job-id-dedupe @ `4df82dddefa8137a2c4c97f2670d0181352c5f64`
**Overall:** CLEAN
...
Three-dot vs origin/dev matches Stages 1–2: config min_company_job_id_match_chars=8 + LENGTH(TRIM) gate on text_matches_known_company_job_id_for_candidate with bind (min_chars, cid, text). Self-Assessment minor matches. Exact job_link dedupe and global helper untouched. Boundaries vs AST-1131/1133 / gaze_email held.
...
- Short junk ids (e.g. 29) excluded from inverted match; length≥8 still matches.
- One merge-tests(AST-1146) SHA; engineer code() is data/utils only.

### Notes

no plan-rubric verdict attached

Docs append on plan file (docs(AST-1146): Radia review — clean).

context_tokens≈28000

#### betty — 2026-08-03T00:12:25.385Z
## QA test manifest

**Publish:** `origin/sub/AST-1130/AST-1146-uat-create-skips-null-company-job-id-dedupe` @ `d6be883a`
`merge-tests(AST-1146): origin/tests 1098755ca49c8c0ed35c43144812d8bb87198c62`

**Broken / obsolete:** none — AST-1132 long-id cases remain eligible (≥8).
**Integration:** none revised.

1. `tests/component/data/database/test_jobs.py::TestAst1146TextMatchesKnownCompanyJobIdMinLength` — short `29` no match; length≥8 still matches; empty/whitespace ignored
2. `tests/component/data/database/test_jobs.py::TestAst1132TextMatchesKnownCompanyJobIdForCandidate` — regression (candidate-scoped long ids)
3. `tests/component/utils/test_config.py::TestAst1146MeteoriteEmailIngestMinCompanyJobIdChars` — `min_company_job_id_match_chars == 8`
4. `tests/component/utils/test_config.py::TestAst1132MeteoriteEmailIngestHygieneConfig` — regression
5. `tests/component/core/test_gazer.py::TestAst1146CreateSkipShortCompanyJobId` — Create body mode: short stored id does not skip; long id still skips
6. `tests/component/core/test_gazer.py::TestAst1061MeteoriteEmailIngest::test_body_mode_skips_known_company_job_id` — regression
...
- `docs/test-bible/data/database/jobs.md` `d8b0973b6daebd9f45ef04ab1cc1678bc7322c17`
- `docs/test-bible/utils/config.md` `5c489661507ab75678463101c8b5bf432ff2f308`
- `docs/test-bible/core/gazer.md` `a618da7b0d2ff3c1bedfce2ad6f5bab0a013ef13`

— Betty

#### hedy — 2026-08-02T23:59:22.186Z
Plan published on `origin/sub/AST-1130/AST-1146-uat-create-skips-null-company-job-id-dedupe` @ `da9d620b`.

**Plan:** https://github.com/susansomerset/astral/blob/sub/AST-1130/AST-1146-uat-create-skips-null-company-job-id-dedupe/docs/features/meteorite/ast-1146-uat-create-skips-null-company-job-id-dedupe.md

**Self-assessment**
- **Scope:** minor — one ingest-config min-length key + `LENGTH(TRIM(company_job_id)) >= min` on `text_matches_known_company_job_id_for_candidate` only.
- **Conf:** high — `matched=29` is the unbounded `LIKE` false-match; config floor excludes short junk without killing exact `job_link` dedupe.
- **Risk:** Medium — too-high floor could miss short legitimate ATS ids; mitigated by keeping `job_link` dedupe and choosing 8 (below UUID length).
