# AST-381 — Pushing database content to GitHub
**Component:** administrator  
**Children:** — (single ticket)  
**Linear archived:** not recorded (no Linear archive block in the source doc)  

## Ledger

_No commits name AST-381. The doc is a plan only — no build trail. (Feature branch named in the doc: `betty/ast-381-pushing-database-content-to-github`.)_

## Epic — AST-381
_Linear URL: https://linear.app/astralcareermatch/issue/AST-381/pushing-database-content-to-github · Project: Astral Administrator (per doc context) · (no Linear archive block in source — archived date / status / assignee / estimate not recorded)_
...
**Done when:** Authenticated admin routes return JSON; import requires `dry_run` first pass boolean; errors return JSON `{"error": "..."}` with appropriate HTTP status per `ASTRAL_CODE_RULES.md` §1.5 UI pattern. _(Historical citation — `docs/ASTRAL_CODE_RULES.md` was later deleted in `44b7d102b`, 2026-09-10.)_
