...
| `src/ui/frontend/src/pages/AdminVectorFeedback.tsx` | Summary + detail `ListPage` sections, filters | ui |
...
1. Create **`src/ui/frontend/src/pages/AdminVectorFeedback.tsx`**:
   - Mirror **`AdminAgentTimesheets.tsx`** structure: **`useSearchParams`**, **`useAdminCandidateFilter`**, **`AdminCandidateFilterControl`**, **`ListPage`**.
...
2. In **`src/ui/frontend/src/routes.tsx`**, import **`AdminVectorFeedback`** and add inside admin routes:
...
3. In **`src/utils/config.py`** **`NAV_CONFIG`**, Admin **`items`**, add after **Agent Timesheets**:
