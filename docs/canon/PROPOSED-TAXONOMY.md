...
| Scope | Governs |
|---|---|
| `general` | applies across every layer |
...
| `ui` | api resolution + React |
| `scripts` | one-off operations, exempt from layer rules |
...
`config → utils` *and* `utils → config` — a cross-layer cycle `s.general.import-rules`
...
`entity` (shapes shared by candidate/company/job), `agent`, `dispatch`,
`consult`, `artifact`.
...
| Pattern | Contents |
|---|---|
| `p.python.logical-scope` | how the backend layers divide; what makes something core vs external vs utils vs config |
| `p.typescript.logical-scope` | how the frontend divides — 94 `.tsx` + 34 `.ts` under `src/ui/frontend`; components vs pages vs contexts vs lib. **Under-derived — see open question 5** |
| `p.core.logical-scope` | core owns orchestration and decides transitions. Module map: `roster` (entity agent story), `tracker` (job + artifacts), `candidate` (candidate lifecycle), `agent` (AI calls), `dispatcher` (scheduling), `consult` (verdicts), `builder`, `gazer`, `intake`, `contact`, `inbox` |
| `p.data.logical-scope` | data owns SQL and raises; never logs, never decides state |
| `p.external.logical-scope` | external performs all I/O, classifies its own failures, returns data |
| `p.utils.logical-scope` | leaf helpers — logging, formatting, auth, network; no business logic |
| `p.config.logical-scope` | the registry: what belongs in `config.py` vs a `*_data` blob vs a DB table |
| `p.ui.logical-scope` | `ui/api/` resolves; React renders what it's handed |

### `data`

| Pattern | Arc / contents | Evidence |
|---|---|---|
| `p.data.entity-definition` | **What is a root column, what goes in the `*_data` JSON, what lives elsewhere.** Root ⟺ the database must query, filter, sort, lock, or transition on it. See §4 for the rubric the schema already follows | `job`, `company`, `candidate` tables; `agent_data` |
| `p.data.save-vs-update` | full upsert vs partial allowlisted update — two writers per entity | `save_company`/`update_company`, `save_agent`/`update_agent`, `save_dispatch_task`/`update_dispatch_task` |

### `entity`

| Pattern | Arc | Evidence |
|---|---|---|
| `p.entity.batch-claim-release` | claim under one `batch_id` → process only claimed rows → clear in `finally` | `claim_{company,job,candidate}_batch`, `clear_*_batch`, `release_job_dispatch_claim`, `_assert_valid_job_batch_claim_state` |
| `p.entity.state-transition` | core picks target from registry → tracker validates `prior_states` → data writes | `transition_job_state`, `_job_state_matches_prior`, `JOB_STATES` |
| `p.entity.artifact-updates` | tag RESPONSE with `entity_id` on write → read latest-per-`task_key` via helper | `list_entity_latest_agent_refs` |

### `artifact`

| Pattern | Arc | Evidence |
|---|---|---|
| `p.artifact.build-lifecycle` | start → cancel → clear, build state on the entity | `start_artifact_build`, `cancel_artifact_build`, `clear_job_build_artifacts`, `BUILD_ARTIFACTS_BASE_STATE` |
| `p.artifact.parse-validate-persist` | shape-check parsed → slice to shape → persist → pin `agent_data` id → hydrate for display | `parsed_matches_artifact_shape`, `slice_parsed_for_artifact_shape`, `persist_job_artifact_from_parsed`, `pin_job_artifact_agent_data_id`, `hydrate_job_artifacts_for_display` |

### `agent`

| Pattern | Arc | Evidence |
|---|---|---|
| `p.agent.prompt-resolution` | task system + task prompts + agent content resolved before the call | `resolved_task_system`, `_resolve_task_prompts`, `resolved_agent_content` |
| `p.agent.chain-context` | caller tokens in → merge for next hop → carry forward | `_incoming_chain_context`, `_chain_context`, `_chain_tokens_for_next_hop`, `_merge_chain_context_for_next_hop`, `_referenced_caller_tokens` |
| `p.agent.response-decode` | stored response text → block by type → inner payload → parsed | `_parsed_response_from_stored_response_text`, `_block_text_by_type`, `_decode_payload`, `_inner_task_payload` |
| `p.agent.grade-validation` | vector names checked → unexpected rejected → grade + confidence validated | `_validate_grade_confidence_list`, `GRADE_VALUES`, `CONFIDENCE_MULTIPLIERS` |

### `dispatch`

| Pattern | Arc | Evidence |
|---|---|---|
| `p.dispatch.ensure-provision` | `ensure_X` idempotent for one entity; `provision_X` seeds template then backfills all | `ensure_meteorite_dispatch_tasks`/`provision_meteorite_dispatch_tasks`, `ensure_gaze_email_dispatch_task`/`provision_gaze_email_dispatch_tasks` |
| `p.dispatch.chain-hop` | `run_next` decides succession → hop label written → graduate at terminal | `write_job_dispatch_hop_label`, `graduate_job_from_dispatch_chain`, `DISPATCH_CHAIN_TERMINAL_GRADUATION` |
| `p.dispatch.scheduler-loop` | tick → circuit breaker → thread target → run/drain/cancel | `_tick_loop`, `start_scheduler`, `_check_circuit_breaker`, `run_task`, `drain_task`, `cancel_task` |
| `p.dispatch.score-floor` | one floor on the `dispatch_task` row governs eligibility *and* soft-fail | existing pattern — keep |
| `p.dispatch.retry-batch` | *(your list — no distinct retry arc found in `src/`; AST-1319 in flight?)* | — |

### `consult`

| Pattern | Arc | Evidence |
|---|---|---|
| `p.consult.verdict-render` | dispatcher loops → `render_verdict` owns per-job lifecycle → tracker never called directly | `_consult_orchestration`, `_render_pass_fail` — **currently a statute with no pattern** |
| `p.consult.rubric-hydration` | criteria from cfg → find criterion → reason for grade → hydrate → snapshot onto job_data | `_rubric_criteria_for_cfg`, `_find_rubric_criterion`, `_lookup_rubric_reason_for_grade`, `_hydrate_grade_reasons_from_rubric`, `_rubric_snapshot_for_job_data` |
| `p.consult.encoded-line` | detect encoded line → parse link index → apply prefilter meta | `_should_decode_as_encoded_line`, `_parse_link_index_field`, `_apply_prefilter_encoded_link_meta` |
...
Seeding *dispatch rows* is `p.dispatch.ensure-provision` above; this scope covers
...
| Pattern | Arc | Evidence |
|---|---|---|
| `p.ui.endpoint` | blueprint route → `@require_auth` → resolve in api → JSON error shape | `api_jobs.py`, `flask.g.user` |
| `p.ui.dirty-leave-save-then-navigate` | existing, still `proposed` — approve it | — |
| `p.config.block` | a `*_CONFIG` dict per feature, registered in `config.py` | `TASK_CONFIG`, `CANDIDATE_CONFIG`, `ROSTER_CONFIG`, ~40 more |
...
| Statute | Rule | From |
|---|---|---|
| `s.variables.local-if-one-off` | single-use values stay local | new |
| `s.variables.named-constants` | magic numbers get named constants with documented meaning | `standards.no-hardcoded-sets` (split) |
| `s.functions.clear-names` | name what it does in stable domain language | `standards.names-not-ticket-ids` |
| `s.functions.non-specific-names` | no ticket ids or ticket-shaped names | same (split) |
| `s.functions.dont-repeat-yourself` | read the whole file before adding; extract shared logic | `standards.dry-and-focused-functions` (split) |
| `s.functions.helper-functions` | complex logic goes to a helper; keep functions focused | same (split) |
| `s.component.file-organization` | public functions first, then helpers, grouped by responsibility | `standards.public-then-helpers` |
| `s.component.comment-hygiene` | — | **new** |
| `s.errors.data-raises-caller-logs` | data raises and never logs; core raises domain; dispatcher catches and logs at batch; UI returns JSON | `standards.data-raises-caller-logs` |
...
| `s.general.import-rules` | ui → core+utils; core → data+external+utils; external → utils; data → utils | `layers.import-direction` |
| `s.general.no-cross-contamination` | nothing outside the five `src/` layers | `standards.no-cross-contamination` |
| `s.general.registry-not-literals` | state lists, enums, allowed sets live in `config.py`; validate against it | `config.config-source-of-truth` + `standards.no-hardcoded-sets` |
| `s.core.entity-save` | **save through the core entity module — `roster` / `tracker` / `candidate`. Do not reach into `data.database` to write, and do not add a new writer** | **new — the seven-bridges rule** |
| `s.core.decides-transitions` | core picks the target state; data/tracker accept it as a parameter | `state.core-decides-transitions` |
| `s.data.table-inventory` | the data layer may use only tables listed in the `database.py` header; changing usage updates the header | `standards.database-header-inventory`, renamed for legibility |
| `s.data.partial-update-allowlist` | `update_X` sets only passed columns, allowlist enforced, returns rowcount | new, from `update_company` |
| `s.data.batch-id-first` | `batch_id` is the first parameter on claim/get/clear helpers | `batch.batch-id-first` |
| `s.data.no-mirror-columns` | no entity-row JSON mirror of `agent_data`; latest refs are read, not stored | `batch.entity-agent-responses-latest-only`, and `company.agent_responses_legacy` is the scar |
| `s.external.io-only-here` | external owns all I/O and returns data | `layers.core-vs-external-bright-line` |
| `s.config.secrets-from-environ` | `os.environ["KEY"]` — no `.get`, no fallback, crash at startup; never a secret in `config.py` | `config.secrets-and-env-specific-from-environ` |
| `s.utils.data-late-import-only` | the one approved `utils → data` path is the log-handler flush | `standards.utils-data-late-import-only` |
| `s.scripts.exempt-from-layer-rules` | `scripts/` may import any layer | `layers.scripts-exempt-from-layer-rules` |
| `s.dispatch.seed-auto-false` | seed paths leave `auto_mode` false | `dispatch.seed-auto-false` |
| `s.dispatch.provision-idempotent` | `ensure_*` is safe to run repeatedly; boot or explicit script only, never per-request | `seed.boot-only-not-hot-path` + your `idempotent-as-default` |
| `s.config.no-shadow-chain` | config must not restate membership or succession already encoded in `agent_task.run_next` | `dispatch.run-next-is-chain-authority` |
| `s.config.single-home` | configuration lives in `config.py` and nowhere else. A module does not define its own public constants — see §12 for the full form | **new** |
...
| `s.state.prior-states-enforced` | transitions raise if the current state is not an allowed predecessor | `state.job-prior-states-enforced` |
| `s.state.one-transition-per-run` | one claim, one process, one transition per cycle; documented `run_next` hops are the carve-out | `state.no-daisy-chain-in-run` |
| `s.agent.confidence-bounds` | integer `confidence` 1–5 with letter grades, 0 with `X`; confidence 1 is no signal | `agent.confidence-bounds` |
| `s.agent.vector-validation` | required vector names present, unexpected rejected, grades in the allowed set | `agent.grade-vector-validation` |
| `s.data.batch-id-format` | `batch_id` is `f"{task_key}-{uuid}"` | `batch.batch-id-format` |
| `s.core.coat-check-never-stores-empty` | a coat-check handler returns `None` rather than storing an empty or failed value, so the next attempt re-fetches | `idioms.coat-check-never-store-empty` |
| `s.ui.icon-control` | compact glyph actions use the shared icon-control class, not one-off buttons or abbreviated text | `pattern.ui.icon-control` — a ui statute, not a pattern |
| `s.ui.shared-button-roles` | labeled buttons use the shared role classes; no parallel families | `pattern.ui.shared-button-roles` — same |
| `s.ui.single-gunicorn-worker` | one gunicorn worker; the dispatch scheduler runs per-worker | `ui.single-gunicorn-worker` |
| `s.scripts.spikes-under-debug` | spike output goes to `debug/spikes/<issue-id>/`, gitignored; findings go on the ticket | `debug.spikes-under-debug-dir` |
| `s.scripts.no-root-artifacts-dir` | no top-level `artifacts/` directory | `debug.no-repo-root-artifacts-dir` |
| `s.agent.core-delegates-io` | core calls `do_task`; config informs; external performs I/O | `agent.do-task-delegation` |
| `s.ui.require-auth` | protected endpoints carry `@require_auth`; identity is `flask.g.user` | `idioms.require-auth-on-protected-endpoints` |
| `s.ui.config-driven-logic` | conditional frontend behaviour resolves in `ui/api/` before serving | `layers.ui-config-driven-business-logic` |
| `s.ui.file-placement` | the prescribed flat frontend locations | `ui.frontend-file-placement` |
| `s.ui.naming-conventions` | PascalCase components; snake_case routes and Python | `ui.naming-conventions` |

---

## 4. `p.data.entity-definition` — the rubric the schema already follows
...
column of exactly the kind `s.data.no-mirror-columns` now forbids. Worth citing
...
**`header-inventory` renamed.** It's now `s.data.table-inventory` — the same
...
survives only as `p.config.block` (the registry *rule* is
`s.general.registry-not-literals`, a statute); and the three
`s.patterns.*` duplicates with their `s.idioms.*` twins.
...
1. **`src/ui/api/api_admin.py` makes 44 direct `database.*` calls.** That's a
   `ui → data` import, which `s.general.import-rules` forbids. It's the only UI
   file doing it. Sanctioned exception, or a real violation to fix?
2. `p.data.save-vs-update` — pattern to follow or wart to remove? It reads
   deliberate (full upsert vs allowlisted partial), but it's the shape that grows
   a third writer.
3. `p.dispatch.retry-batch` — no distinct retry arc found in `src/`. AST-1319
   still in flight?
...
| Current | Becomes |
|---|---|
| `batch.claim-process-release` + `pattern.batch.entity-claim-process-release` | `p.entity.batch-claim-release` (the merge) |
| `batch.entity-agent-responses-latest-only` + `pattern.batch.entity-agent-responses` | `p.entity.artifact-updates` (the merge) |
| `pattern.state.entity-state-transitions` | `p.entity.state-transition` |
| `idioms.render-verdict-orchestrates-consult` | `p.consult.verdict-render` |
| `pattern.ui.admin-endpoint` | `p.ui.endpoint` |
| `pattern.config.config-block` | `p.config.block` |
| `agent.do-task-delegation` | `s.agent.core-delegates-io` |
...
```bash
python3 canon/canon_clerk.py index --json
```
...
| Dispatch / state policy | 11 | `effective_dispatch_score_floor`, `dispatch_claim_states`, `dispatch_chain_graduation_target`, `is_valid_job_batch_claim_state` | `core` — these are decisions, and `s.core.decides-transitions` says core makes them |
...
| UI manifest builders | 4 | `build_state_ui_manifest`, `jobs_ui_rubric_for_state`, `admin_hidden_dispatch_task_keys` | `ui/api/` — `s.ui.config-driven-logic` already says resolve there |
...
| Statute | Rule |
|---|---|
| `s.config.data-not-behaviour` | config holds values and plain accessors over them. Decisions, codecs, templating, and response shaping live in the layer that owns them. If a config function needs to import a peer to do its job, it isn't config |
...
5. **The TypeScript half is under-derived.** My first survey globbed `*.py` and
   ...
   `src/ui/api/` (the Python side) only. `p.typescript.logical-scope`,
   `s.ui.icon-control`, and `s.ui.shared-button-roles` need a pass over the real
   frontend before they're trustworthy.
6. **Does splitting `general` into `python` / `typescript` split the construct
   statutes too?** `s.functions.clear-names` and `s.component.file-organization`
   read as language-neutral, but `s.ui.naming-conventions` currently straddles
...
`s.ui.config-driven-logic` most of it shouldn't be in `config.py` at all; it
...
computing them. That is §8's `s.config.data-not-behaviour` measured from a
different angle.
...
the one `s.general.registry-not-literals` is asking for.
...
| Directive | Rule |
|---|---|
| `s.config.every-block-states-its-scope` | every top-level block carries a one-line scope statement saying what belongs in it. A new value goes in the block whose scope covers it; if none does, it goes in `ASTRAL_CONFIG`. Adding a *new* top-level block requires Archie |
| `s.config.derive-dont-restate` | a subset of a registry is derived from it, never typed out again. `IN_REVIEW_STATES` and `SKIPPED_STATES` are the current violations |
| `p.config.task-config` | one file, two sections. `# Logical scope`: what belongs in `TASK_CONFIG`. `# Data coupling`: what the scorer compares — `TASK_CONFIG` keys against `agent_task` rows, kept in sync by `data/admin/agent_task.json` — and what each band means. **Measured today: 47 config keys vs 52 JSON keys, 44 shared** |
...
`s.config.derive-dont-restate` already being obeyed. It's large because there are
...
### `p.config.task-config` — the coupling worth writing down
...
## 12. `s.config.single-home` — worked example of the beefy format
...
| | |
|---|---|
| **5** | new value added to the correct root node in `config.py`, imported where used |
| **4** | in `config.py` but under a debatable root node |
| **3** | public module constant that is arguably implementation — unclear call, discuss |
| **2** | public behaviour-driving constant defined in a module |
| **1** | a module-local config block, or a second module importing another module's constant |
...
That is precisely why it carries `p.config.task-config` with a `# Data coupling`
