...
> **Status.** `canon/canon_clerk.py` is built and working. Everything else here
...
> target, not the current state. See [PROPOSED-TAXONOMY.md](PROPOSED-TAXONOMY.md)
...
One scale, both stages, terse output. The score line is just `slug: n`, because
the directive text is already enumerated in the description.

| Score | Meaning | Routes to |
|---|---|---|
| **5** | implemented as the directive directs | PROCEED |
| **4** | varied slightly, within the directive's scope | PROCEED |
| **3** | diverges from the directive's intention | DISCUSS |
| **2** | ignores the directive | FIX NOW |
| **1** | explicitly builds a separate path to the same result | FIX NOW |

**Aggregation is min-wins. Never average.** Eight 5s and a 1 is a FIX NOW, not a
4.6. Any 1 or 2 → FIX NOW; any 3 → DISCUSS; otherwise PROCEED.

**5 and 4 are bare; 3, 2, and 1 carry one line naming the file or decision.** The
score carries the verdict, but location isn't recoverable from the description,
so findings pay for a line and passes don't.
...
`canon/canon_clerk.py`. Two commands, matching the two things agents need.

```bash
python3 canon/canon_clerk.py index --kind statute
python3 canon/canon_clerk.py expand <id> [<id> ...]
```
...
miniature, and it violates `s.general.registry-not-literals` by the governance
...
- [PROPOSED-TAXONOMY.md](PROPOSED-TAXONOMY.md) — the directive-level target:
