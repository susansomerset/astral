...
All batch jobs that process entities by state use batch locking. Every `ENTITY_TYPES` member used as a dispatch claim queue (`candidate`, `company`, `job`, `meteorite`) uses the same pool claim → process → release shape. Candidate is not exempt: no unlocked single-ctx claim path, no empty release stub. The `batch_id` is the **golden ticket** — one ID per dispatch run that ties together row-locking, state transitions, agent_data blocks (including RESPONSE `entity_id` latest-per-task refs), dispatch_ledger entries, and timesheets.
...
**Data (`src/data/`):** Database and file I/O only. No business logic, no external API calls, no UI. Exposes focused functions. Paths from config. **`agent_data.block_data` is zlib-compressed on write and decompressed on read — this is handled transparently by `save_agent_data` / `get_agent_data_by_batch` / `get_agent_data` / `get_agent_data_for_ids`. Writes may set nullable `ref_agent_data_id` to the earliest identical content row and omit duplicate payload on the audit row; reads follow that ref before returning plain text. Callers always receive plain text strings; compression and ref resolution are invisible above the data layer.** **`artifact.artifact_data` uses the same zlib compress-on-write / decompress-on-read pattern via `save_artifact` / `get_current_artifact` / `get_artifact` / `list_artifacts` (AST-1605); callers still receive deserialized JSON/plain values; legacy uncompressed TEXT rows remain readable.**
