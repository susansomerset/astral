...
**Broken / obsolete (revised this pass):** `test_ArtifactEditor.test.tsx` AST-645 `toHaveClass("save")` → `btn` + `primary`. `[qa-handoff]` harness: stub `setAuthTokenGetter` / `setUnauthorizedHandler` on partial `lib/api` mocks (`JobDetailModal`, `CompaniesNewList`, `AdminCostReconciliation`); `ok: true` on Agent Prompts GETs; AST-634 filter fixture uses top-level `first` (labels are `c.first`, not `candidate_data.first`). After ftr/1302 merge-resume, glyph rows are `icon-control` — still do **not** run **`AST-1302:`** names on this ticket.
