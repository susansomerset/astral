...
| **AST-682** | AST-675 child: env label native `title` lists up to **20** `merge_tickets` (`ticket_id` + `fmtTime(recorded_at)`), newline-separated, API order (most recent first); no `title` when `merge_tickets` absent/empty; uptime unchanged | `AdminDeployFooter.tsx` | **`test_AdminDeployFooter.test.tsx`** — **`test_sets_merge_ticket_tooltip_on_environment_label_when_merge_tickets_present`**; **`test_omits_title_on_environment_label_when_merge_tickets_empty_or_missing`**; **`test_caps_merge_ticket_tooltip_at_20_lines`**; existing env/uptime/error tests unchanged |
