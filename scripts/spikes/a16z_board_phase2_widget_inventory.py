#!/usr/bin/env python3
"""
AST-401 Phase 2: first-screen input-related controls on jobs.a16z.com/jobs → widgets.json.

Also walks `button.block-tray-toggle` (a16z-only) and captures option rows / pills / salary panel text.

Usage (repo root, PYTHONPATH=.):
  python3 scripts/spikes/a16z_board_phase2_widget_inventory.py [--headed] [--url URL] [--out PATH]
"""
...
async def _dismiss_consent(page: Page) -> Optional[str]:
    """Same ordered strategy as Phase 1 (AST-400); duplicated here to stay under DRY extract threshold."""
    try:
...
async def _collect_block_tray_option_lists(page: Page) -> List[Dict[str, Any]]:
    """
    Open each `button.block-tray-toggle` on jobs.a16z.com/jobs and read whatever option UI appears.
...
def main() -> None:
    ...
    p.add_argument(
        "--out",
        default=str(_ROOT / "debug/spikes/boards/a16z/phase2/widgets.json"),
