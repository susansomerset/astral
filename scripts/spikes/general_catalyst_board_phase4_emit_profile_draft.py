#!/usr/bin/env python3
"""
AST-433 Phase 4: assemble board_profile_draft.json from Phase 1–3 spike artifacts (stdlib only).

Usage (repo root):
  python3 scripts/spikes/general_catalyst_board_phase4_emit_profile_draft.py
  python3 scripts/spikes/general_catalyst_board_phase4_emit_profile_draft.py --mirror-inputs
  python3 scripts/spikes/general_catalyst_board_phase4_emit_profile_draft.py \\
    --phase1-dir artifacts/general_catalyst/phase1/run \\
    --widgets-json artifacts/general_catalyst/phase2/widgets.json \\
    --phase3-dir artifacts/general_catalyst/phase3/run
"""
...
DEFAULT_OUT = _ROOT / "debug/spikes/AST-433/board_profile_draft.json"
BOARD_KEY = "general-catalyst"
ENTRY_URL = "https://jobs.generalcatalyst.com/jobs"
LABEL = "General Catalyst Jobs"
...
PHASE1_CANDIDATES = [
    _ROOT / "debug/spikes/AST-430",
    _ROOT / "debug/spikes/AST-430/phase1/run",
    _ROOT / "debug/spikes/general_catalyst/phase1/run",
]

WIDGETS_CANDIDATES = [
    _ROOT / "debug/spikes/AST-431/widgets.json",
    _ROOT / "debug/spikes/general_catalyst/phase2/widgets.json",
]

PHASE3_CANDIDATES = [
    _ROOT / "debug/spikes/AST-432",
    _ROOT / "debug/spikes/AST-432/phase3/run",
    _ROOT / "debug/spikes/general_catalyst/phase3/run",
]

LEGACY_PHASE1 = _ROOT / "artifacts/general_catalyst/phase1/run"
LEGACY_WIDGETS = _ROOT / "artifacts/general_catalyst/phase2/widgets.json"
LEGACY_PHASE3 = _ROOT / "artifacts/general_catalyst/phase3/run"
OLDER_ROOT = _ROOT / "debug/spikes/older/general_catalyst"
    ...
    *,
    ...
    if not widgets.get("inline_tray_option_lists"):
        gaps.append(
            _gap(
                ...
                "warn",
                "widgets.json has no inline_tray_option_lists; re-run general_catalyst_board_phase2_widget_inventory.py",
            )
        )
    ...
    draft: Dict[str, Any] = {
        ...
        "source_artifacts": {
            ...
            "AST-431": {"widgets.json": str(widgets_path.resolve())},
            "AST-432": {
...
def main() -> None:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  Phase 1: debug/spikes/AST-430/, AST-430/phase1/run/, general_catalyst/phase1/run/\n"
            "  Phase 2: debug/spikes/AST-431/widgets.json, general_catalyst/phase2/widgets.json\n"
            "  Phase 3: debug/spikes/AST-432/, AST-432/phase3/run/, general_catalyst/phase3/run/\n"
            "  --allow-legacy-paths: also artifacts/general_catalyst/… and debug/spikes/older/general_catalyst/…\n"
            "New output must use debug/spikes/AST-433/ (not artifacts/ or general_catalyst/phase4)."
        ),
    )
    ...
    p.add_argument(
        "--allow-legacy-paths",
        action="store_true",
        help="Search artifacts/general_catalyst/ and debug/spikes/older/ after defaults",
    )
    p.add_argument(
        "--mirror-inputs",
        action="store_true",
        help="Copy resolved inputs to debug/spikes/AST-433/inputs/",
    )
    args = p.parse_args()
    ...
    if phase1 is None:
        gaps.append(
            _gap(
                "phase1_missing",
                "error",
                "Phase 1 dir not found; re-run general_catalyst_board_phase1_reach.py --out-dir debug/spikes/AST-430/ "
                "or pass --phase1-dir",
            )
        )
    if widgets is None:
        gaps.append(
            _gap(
                "widgets_missing",
                "error",
                "widgets.json not found; re-run phase2 with --out debug/spikes/AST-431/widgets.json "
    ...
    if phase3 is None:
        gaps.append(
            _gap(
                "phase3_missing",
                "error",
                "Phase 3 dir not found; re-run phase3 with --out-dir debug/spikes/AST-432/ "
                "or pass --phase3-dir",
            )
        )
