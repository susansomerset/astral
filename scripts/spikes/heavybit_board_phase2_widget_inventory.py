#!/usr/bin/env python3
"""
AST-423 Phase 2: first-screen input-related controls on heavybit.com/jobs → widgets.json.

Fork of a16z Phase 2 collectors (`block-tray-toggle`, inline trays, typeahead, select-pill, salary panel).
Heavybit-specific selector overrides go in script header only if Phase 1 `visible.txt` diverges.

Usage (repo root, PYTHONPATH=.):
  python3 scripts/spikes/heavybit_board_phase2_widget_inventory.py [--headed] [--url URL] [--out PATH]
"""
...
DEFAULT_URL = "https://heavybit.com/jobs"
...
async def _dismiss_consent(page: Page) -> Optional[str]:
    """Same ordered strategy as Phase 1 (AST-422); duplicated here to stay under DRY extract threshold."""
    try:
...
async def _collect_block_tray_option_lists(page: Page) -> List[Dict[str, Any]]:
    """
    Open each `button.block-tray-toggle` on heavybit.com/jobs and read whatever option UI appears.
...
def main() -> None:
    ...
    p.add_argument(
        "--out",
        default=str(_ROOT / "debug/spikes/AST-423/widgets.json"),
