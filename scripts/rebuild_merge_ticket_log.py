...
Usage:
    python3 scripts/rebuild_merge_ticket_log.py [--dev-ref origin/dev]
"""
...
def _collect_entries(dev_ref: str, uat_state_name: str) -> list[dict]:
    entries: list[dict] = []
    for parent_id in fetch_user_testing_parent_ids(uat_state_name=uat_state_name):
...
def main() -> None:
    ...
    try:
        entries = _collect_entries(args.dev_ref, uat)
    except (LinearApiError, RuntimeError, ValueError) as exc:
        print(str(exc), file=sys.stderr)
        sys.exit(1)
    rebuild_merge_ticket_log(entries)
    summary = {"count": len(entries), "parents": [e["ticket_id"] for e in entries]}
